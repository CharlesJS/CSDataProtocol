...
// https://github.com/apple/swift-corelibs-foundation/blob/main/Sources/Foundation/DataProtocol.swift
...
extension DataProtocol {
    ...
    private func matches<D: DataProtocol>(_ data: D, from index: Index) -> Bool {
        ...
        while true {
            guard self[haystackIndex] == data[needleIndex] else { break }
            ...
            // Since the length is checked in every caller of this code, haystackIndex should never actually == endIndex.
            // Change the check to an assertion to make it possible to get 100% code coverage
            assert(haystackIndex != endIndex)
        }
    ...
    public func lastRange<D: DataProtocol, R: RangeExpression>(of data: D, in range: R) -> Range<Index>? where R.Bound == Index {
        ...
        while true {
            ...
            // The code in swift-corelibs-foundation crashes with an index out of bounds error when the term is not found.
            // Fix this by not decrementing when we hit the lower bound.
            guard position > r.lowerBound else {
                break
            }
...
extension DataProtocol where Self : ContiguousBytes {
    public func copyBytes<DestinationType, R: RangeExpression>(to ptr: UnsafeMutableBufferPointer<DestinationType>, from range: R) where R.Bound == Index {
        ...
        withUnsafeBytes { fullBuffer in
            ...
            UnsafeMutableRawPointer(ptr.baseAddress!).copyMemory(
                from: fullBuffer.baseAddress!.advanced(by: adv),
                byteCount: delta
            )
        }
    }
...
extension Slice : DataProtocol where Base : DataProtocol {
    ...
    public var regions: [Base.Regions.Element.SubSequence] { self._regions }

    public var _regions: [Base.Regions.Element.SubSequence] {
